		...
		class Parallaxsome_Welcome {
			...
			public function parallaxsome_activation_admin_notice() {
				...
				if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) {
					...
					<div class="notice notice-success is-dismissible">
						<p><?php printf( __( 'Welcome! Thank you for choosing %1$s! Please make sure you visit our <a href="%2$s">Welcome page</a> to get started with %1$s.', 'parallaxsome' ), $this->theme_name, admin_url( 'themes.php?page=parallaxsome-welcome' )  ); ?></p>
						<p><a class="button" href="<?php echo admin_url( 'themes.php?page=parallaxsome-welcome' ) ?>"><?php _e( 'Lets Get Started', 'parallaxsome' ); ?></a></p>
					</div>
					<?php
				}
			}
			...
			public function parallaxsome_welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
					<h1><?php printf( esc_html__( 'Welcome to %s - Version %s', 'parallaxsome' ), $this->theme_name, $this->theme_version ); ?></h1>
					<div class="about-text"><?php printf( esc_html__( '%s is a beautiful free responsive one page WordPress theme with awesome parallax scrolling effect. It is a highly customizable and flexible WP template suitable for multipurpose websites like business, agency, blog, portfolio, eCommerce etc. It is a fully responsive theme  that fits perfectly on any device. ParallaxSome theme comes with various homepage sections each beautifully designed.', 'parallaxsome' ), $this->theme_name ); ?></div>
			...
			public function parallaxsome_plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to install plugins on this site.', 'parallaxsome' ) );
				...
				if (! wp_verify_nonce( $nonce, 'parallaxsome_plugin_installer_nonce' ))
					wp_die( __( 'Error - unable to verify nonce, please try again.', 'parallaxsome') );
			...
			public function parallaxsome_plugin_activation_callback(){
				...
				if ( ! current_user_can('install_plugins') )
					wp_die( __( 'Sorry, you are not allowed to activate plugins on this site.', 'parallaxsome' ) );
				...
				if (! wp_verify_nonce( $nonce, 'parallaxsome_plugin_activate_nonce' ))
					die( __( 'Error - unable to verify nonce, please try again.', 'parallaxsome' ) );
